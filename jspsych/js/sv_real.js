...
var debrief_block = {
  ...
  stimulus: function () {
    ...
    var fixations = jsPsych.data.get().filter({ cond:'vis_s', test_part: 'fixation' });
    ...
    var correct_real = jsPsych.data.get().filterCustom(
      function (trial) {
        return (trial.cond == "vis_s") && (trial.test_part == "test") && (trial.key_press === 32) && (trial.rt > 100);
      }
    )
    ...
    return "<p>Din gennemsnitlige reaktionstid var </p>" +
      ...
      "<p>Du kan finde din reaktionstid på skalaen her, og se om du var hurtigere eller langsommere end andre, der har taget denne online-test:</p>" +
            "<div class='center-img'><img src='img/vis_feed.png'></img></div>" +
  ...
  on_finish: function (data) {
    // get data
    var trials = jsPsych.data.get().filter({ cond:'vis_s', test_part: 'test' });
    var fixations = jsPsych.data.get().filter({ cond:'vis_s', test_part: 'fixation' });
    var false_alarms = fixations.filter({ correct: false });
    // exclude trials with response time smaller than 100 ms (considered false anticipatory responses)
    var correct_real = jsPsych.data.get().filterCustom(
